...
def after_request(response):
	...
	response.headers.add('Content-Security-Policy', 
						 'default-src \'self\' \'unsafe-eval\'; \
						  ...
						  script-src \'strict-dynamic\' cdn.jsdelivr.net \'nonce-' + getattr(request, '__nonce', str()) + '\' \'unsafe-inline\'; \
						  object-src \'none\'; \
						  base-uri \'none\'; \
	...
	if app.config.get('APP_DEBUG'):
		...
		logging.debug("[%s/%s] %s %s", request.method, response.status_code, request.remote_addr, request.path)
	return response
